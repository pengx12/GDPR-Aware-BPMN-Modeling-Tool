...
let Papa = require('../node_modules/papaparse');
// import * as URI from "uri-js";
const URI = require("urijs");
// import data from '../data.mysql';
// let connectdatabase = require("../RDBtoRDF/testdatabase");
// connectdatabase('testtable');
var RDFJSInterface = require('rdf_js_interface');
// let URI = require('../node_modules/uri-js');
// let csvString = require('../RDBtoRDF/test.csv');
/*
Dependencies:
...
var default_meta = function (data, url, headers) {
  ...
  if (headers) {
    ...
    for (var i = 0; i < data.length; i++) {
      retval.schema.columns.push({ name: data[i], "@type": "Column" });
    }
...
var rdf_conversions = {
  ...
  addsubject: function (state, subject, predicate, object) {
    // See how the URI for the predicate is set up
    var s = state.rdf.createNamedNode(subject.value)
    var p = state.rdf.createNamedNode(predicate.value)
    if (object.isuri) {
      var o = state.rdf.createNamedNode(object.value)
    } else {
    ...
    state.graph.add(state.rdf.createTriple(s, p, o));
  },
};
...
var c_default = function (data, meta, target_format, warnings) {
  ...
  var url = new URI(meta["@id"]).normalize().toString() + '/' + new URI(tablename).normalize().toString();
  ...
  conv_functions.start(state, data['colname'], meta, warnings);
  ...
  meta.schema.columns.forEach(function (col, index) {
    ...
    col_names.push({
      name: col.name,
      uri: state["@base"] + new URI(col.name).normalize().toString(),
      isURI: true,
      index: index
    });
  ...
  // Go through each row:
  // data['colname'].forEach( function(data_row, rindex) {
  ...
  // })
  var rid = new URI(tablename).normalize().toString();
  // console.log(rid);
  ...
  if (data['type'] == 'database') {
    let databasename = data['database']
    var subject = {
      // value  : col_names[cindex].uri,
      value: new URI(meta["@id"]).normalize().toString() + '/' + new URI(databasename).normalize().toString(),
    };
    var predicate = {
      // value  : col_names[cindex].uri,
      value: 'http://kdeg.scss.tcd.ie/gdpr-bpmn/ontology#hasTable'
    };
    // This is where the datatype issue will come in!
    var object = {
      // value : cell,
    ...
    conv_functions.addsubject(state, subject, predicate, object);
    var predicate = {
      value: "csv:name",
    };
    // This is where the datatype issue will come in!
    var object = {
      value: new URI(meta["@id"]).normalize().toString()+'/' + databasename,
      isuri: false,
    }
    conv_functions.addsubject(state, subject, predicate, object);
  }
  ...
  const predictVal = 'columns'
  data['colname'].forEach(function (cell, cindex) {
    var predicate = {
      // value  : col_names[cindex].uri,
      value: new URI(predictVal).normalize().toString(),
      prefix: "csv",
    };
    // This is where the datatype issue will come in!
    var subject = {
  ...
  data['colname'].forEach(function (cell, cindex) {
    ...
    var rid = new URI(cell).normalize().toString();
    // console.log(rid);
    conv_functions.new_row(state, rid);
    conv_functions.add_type(state, "Column")
    // let tmp=data['notsensitive']
    ...
    if (!(('notpersonal' in data) && (data['notpersonal'].indexOf(cell) != -1))) {
      ...
      var subject = {
        // value : cell,
        value: "https://w3id.org/GDPRtEXT/PersonalData",
        isuri: true,
      }
      conv_functions.add(state, predicate, subject);
    }
...
*/
export let getCSV = function (options, csvString) {
  // let getCSV = function(options,csvString, success, failure) {
  //var result = Deferred();
  var result;
  var final_warnings = [];
  ...
  // let csvString=[
  //   {"Col 1":"aaa","Col 2":"bbb","Col 3":"2014-02-24"},
  //   {"Col 1":"123","Col 2":"345","Col 3":"ddd"}
  //   ]
  // let csvString={
  //   'tablename':"InformationTable",
  //   'colname':["name","email","gender","birthday","phone number"],
  //   'notsensitive':['name']
  // }
  //   var str=JSON.stringify(csvString);
  // // var pcsv = Papa.parse(csvString);
  // var pcsv=csvString
  ...
  var embedded_meta = default_meta(csvString['colname'], url, true);
  ...
  // });    
  let finalformat = CSV_format.TURTLE
  var final_data = convertCSV(csvString, embedded_meta, "", finalformat, final_warnings);
  // retval = {
  //   data      : final_data,
  //   meta      : embedded_meta,
  //   errors    : final_warnings.concat(csvString.errors)
  // }
  // success_callback(retval);
...
export default getCSV
    ...
    //   'notsensitive':['name']
    // }
    // getCSV("http://foo.bar/database",csvString);
    // getCSV("http://foo.bar/database",csvString, function(csv_data) {
    //        // display the CSV data; by default, the data is a javascript object
    //        display(csv_data.data["Col1"]);
    //        display(csv_data.data["Col2"]);
    //      });